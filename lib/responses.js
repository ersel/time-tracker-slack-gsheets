...
const responses = {
	...
	commandHelp() {
		let response = [];
		response.push(`*Time track usage:*`);
		response.push('```');
		response.push('To track an activity: ');
		response.push('/track COMPANYNUMBER "activity name or alias" duration');
		response.push('Duration examples: 01:11:20 or 11:20 or 1h or 1m or 1s');
		response.push('eg: /track 12345678 "called the client" 12m');
		response.push('');
		response.push('To get the spreadsheet URL:');
		response.push('/track url');
		response.push('');
		response.push('To see a list of activities:');
		response.push('/track list');
		response.push('```');
		return response.join('\n');
	},
